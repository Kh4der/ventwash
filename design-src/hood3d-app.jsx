// Hood3DApp — scroll-driven Three.js assembly/cleaning experience.
// Globals: THREE (CDN), React, useTweaks/TweaksPanel/etc from tweaks-panel.jsx.
/* global THREE, React */
...
const PART_SEC = { canopy: 2, filters: 3, fire: 4, duct: 5, fan: 6, mua: 7 };
...
function buildFlow(scene) {
  const nEx = 90, nIn = 40, total = nEx + nIn;
  ...
  for (let i = 0; i < total; i++) {
    const inFlow = i >= nEx;
    meta.push({ phase: Math.random(), jx: (Math.random() - 0.5), jz: (Math.random() - 0.5), inFlow });
    const c = inFlow ? [0.42, 0.62, 0.9] : [0.62, 0.66, 0.72];
    col[i * 3] = c[0]; col[i * 3 + 1] = c[1]; col[i * 3 + 2] = c[2];
  }
  geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
  geo.setAttribute('color', new THREE.BufferAttribute(col, 3));
  const m = new THREE.PointsMaterial({ size: 0.055, vertexColors: true, transparent: true, opacity: 0, depthWrite: false, blending: THREE.AdditiveBlending });
  ...
  return {
    ...
    update(time, alpha) {
      ...
      for (let i = 0; i < total; i++) {
        const d = meta[i];
        const u = (time * 0.12 + d.phase) % 1;
        let x, y, z;
        if (!d.inFlow) {
          if (u < 0.35) { const v = u / 0.35; x = lerp(d.jx * 1.6, 0, v * v); y = lerp(1.15, 2.3, v); z = lerp(d.jz * 0.7, 0, v); }
          else if (u < 0.72) { const v = (u - 0.35) / 0.37; x = Math.sin(v * 8 + d.phase * 9) * 0.08; y = lerp(2.3, 3.9, v); z = d.jz * 0.1; }
          else { const v = (u - 0.72) / 0.28; x = d.jx * 1.1 * v; y = lerp(3.9, 5.2, v); z = d.jz * 1.1 * v; }
        } else {
          const v = u;
          x = lerp(-1.62, d.jx * 1.2, v);
          y = lerp(3.05, 1.3, v * v);
          z = lerp(0.45, 0.5 + d.jz * 0.6, v);
        }
...
function MeshGradientBG({ speed, swirl }) {
  ...
  React.useEffect(() => {
    const c = cnv.current;
    const gl = c.getContext('webgl', { antialias: false, depth: false, stencil: false, preserveDrawingBuffer: true });
    if (!gl) return;
...
function WipeBtn({ label, primary, small, onClick }) {
  ...
  return <div ref={ref} data-hover="1" onClick={onClick} onMouseEnter={() => setH(true)} onMouseMove={onMove} onMouseLeave={onLeave}
    style={{ position: 'relative', overflow: 'hidden', display: 'inline-block', padding: small ? '10px 18px' : '14px 28px', borderRadius: 3, cursor: 'none', fontWeight: 700, fontSize: small ? 13.5 : 15, userSelect: 'none', transition: 'transform .3s cubic-bezier(.22,1,.36,1)', background: primary ? '#1a2129' : 'transparent', border: primary ? 'none' : '1.5px solid #1a2129' }}>
...
function Hood3DApp() {
  ...
  React.useEffect(() => {
    ...
    function onPD(e) { if (e.pointerType === 'mouse' && e.button !== 0) return; drag.on = true; drag.moved = 0; drag.x = e.clientX; drag.y = e.clientY; const st = stateRef.current; st.uyV = 0; st.upV = 0; }
    function onPM(e) {
      if (!drag.on) return;
    ...
    function init() {
      if (disposed) return;
      if (!window.THREE) { setTimeout(init, 60); return; }
    ...
    function loop(tms) {
      ...
      const st = stateRef.current;
      st.cur = lerp(st.cur, st.target, 0.075);
      const P = st.cur;
      ...
      // explode factor
      const E = sstep(0, 1, sectionLocal(P, 1)) * (1 - sstep(0.05, 0.9, sectionLocal(P, 8)));
      // per-part
      ...
      Object.entries(parts).forEach(([id, p]) => {
        ...
        else wipe = sstep(0.15, 0.75, sectionLocal(P, 8)); // range cleans during rebuild
      ...
      // idle drift at hero & finale
      const idle = (1 - sstep(0, 0.5, segF)) + sstep(N - 1.6, N - 1.05, segF);
      ...
      // flow particles in finale
      const flowA = sstep(0.1, 0.45, sectionLocal(P, 9)) * (t.airflow === false ? 0 : 1);
      flow.update(time, flowA);
      ...
      if (U.dot && mouse.px != null) {
        U.dot.style.transform = `translate(${mouse.px - 4}px, ${mouse.py - 4}px)`;
        st2.rx = lerp(st2.rx, mouse.px, 0.16); st2.ry = lerp(st2.ry, mouse.py, 0.16);
        st2.hs = lerp(st2.hs, (hoverRef.current || drag.on) ? 2.1 : 1, 0.18);
        U.ring.style.transform = `translate(${st2.rx - 18}px, ${st2.ry - 18}px) scale(${st2.hs.toFixed(3)})`;
      }
      ...
      if (U.mark) { const mo = sstep(0.15, 0.6, sectionLocal(P, 9)); U.mark.style.opacity = (mo * 0.9).toFixed(3); U.mark.style.transform = `translateY(${((1 - mo) * 40).toFixed(1)}px)`; }
      ...
      if (U.mAin) {
        ...
        U.mA.style.opacity = (sstep(0.72, 1.05, pn) * (1 - sstep(1.75, 2.15, pn))).toFixed(3);
      }
      if (U.mBin) {
        const w = U.mBin.scrollWidth / 2 || 1;
        U.mBin.style.transform = `translateX(${((mdrift * 0.8 + vel * 3000) % w - w).toFixed(1)}px)`;
        U.mB.style.opacity = (sstep(7.85, 8.35, pn) * (1 - sstep(9.1, 9.5, pn))).toFixed(3);
      }

      // hover raycast + part highlight
      if (pickMeshes && pickMeshes.length && !drag.on && mouse.px != null) {
      ...
      // 3D part labels (appear on the active cleaning section, on explode, or on hover)
      {
        ...
        const showSet = {};
        if (ai === 1 && E > 0.5) ORDER3D.forEach(id => { if (id !== 'range') showSet[id] = 1; });
        ...
        ORDER3D.forEach((id, li) => {
          ...
          if (show !== !!shown[id]) {
            shown[id] = show;
            const g2 = window.gsap, dot = U['lbd_' + id], chip = U['lbc_' + id];
            if (g2 && dot && chip && ln) {
      ...
      if (U.rot) U.rot.style.opacity = ((pn > 0.85 && pn < 8.6) ? 0.85 : 0).toFixed(2);
    ...
    return () => {
      ...
      if (mountRef.current) mountRef.current.removeEventListener('pointerdown', onPD);
  ...
  React.useEffect(() => {
    if (!loaded) return;
    window.scrollTo(0, 0);
    if (window.gsap) window.gsap.fromTo('[data-hdr]', { y: -28, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: 1, ease: 'expo.out', stagger: 0.09, delay: 0.25 });
  }, [loaded]);
  ...
  return (
    <div style={{ fontFamily: "'Archivo',sans-serif", color: '#1a2129', cursor: 'none' }}
      ...
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px clamp(20px,4vw,44px)' }}>
        ...
        <div data-hdr="1" style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <WipeBtn primary small label="Get a free quote" />
        </div>
      </header>
          ...
          <div key={s.id} ref={(el) => { capRefs.current[i] = el; }} data-screen-label={s.id} style={capStyle(s)}>
            ...
            {s.cta ? <RevealBlock active={act} delay={0.5} style={{ marginTop: 30, pointerEvents: 'auto' }}>
              <div style={{ display: 'inline-flex', gap: 12 }}>
                <WipeBtn primary label="Get a free quote" />
                <WipeBtn label="Call (800) VENT-WASH" />
              </div>
      ...
      <div style={{ position: 'fixed', right: 22, top: '50%', transform: 'translateY(-50%)', zIndex: 5, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {SECTIONS.map((s, i) => (
          <div key={s.id} ref={(el) => { dotRefs.current[i] = el; }} onClick={() => scrollToSec(i)} title={s.kicker} data-hover="1"
            style={{ width: 9, height: 9, borderRadius: '50%', background: 'rgba(30,40,50,.25)', cursor: 'none', transition: 'transform .2s, background .2s' }}></div>
        ))}
      </div>
      ...
      <div ref={(el) => { uiRefs.current.dot = el; }} style={{ position: 'fixed', left: 0, top: 0, width: 8, height: 8, borderRadius: '50%', background: '#1a2129', zIndex: 70, pointerEvents: 'none' }}></div>
      <div ref={(el) => { uiRefs.current.ring = el; }} style={{ position: 'fixed', left: 0, top: 0, width: 36, height: 36, borderRadius: '50%', border: '1.5px solid rgba(26,33,41,.5)', zIndex: 70, pointerEvents: 'none' }}></div>
      ...
      <TweaksPanel>
        ...
        <TweakSlider label="Grease level" value={tw.grease} min={0.3} max={1.4} step={0.05} onChange={(v) => setTweak('grease', v)} />
        <TweakToggle label="Airflow particles" value={tw.airflow !== false} onChange={(v) => setTweak('airflow', v)} />
        <TweakSection label="Background" />
        <TweakSlider label="Flow speed" value={tw.bgSpeed ?? 0.8} min={0} max={2} step={0.05} onChange={(v) => setTweak('bgSpeed', v)} />
        <TweakSlider label="Swirl" value={tw.bgSwirl ?? 0.6} min={0} max={1.5} step={0.05} onChange={(v) => setTweak('bgSwirl', v)} />
      </TweaksPanel>
    </div>
  );
}

window.Hood3DApp = Hood3DApp;
